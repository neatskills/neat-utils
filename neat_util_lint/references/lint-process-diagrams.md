...
```mermaid
graph TD
    Q1{Run Phase 1?} -->|yes| A[Phase 1: Structure]
    Q1 -->|no| Q2
    A --> B{FAIL items?}
    B -->|yes| C[Fix and re-run Phase 1]
    B -->|no| Q2
    C --> A
    Q2{Run Phase 2?} -->|yes| D[Phase 2: Logic Flow]
    Q2 -->|no| Q3
...
```mermaid
graph TD
    A[Phase 1: Structure — checks 1-2, 5-11] --> B{FAIL items?}
    B -->|yes| C[Fix and re-run Phase 1]
    C --> A
    ...
    J --> K{Zero warnings?}
    K -->|no, iteration < 3| I
    K -->|yes or max iterations| L[Done]
```
